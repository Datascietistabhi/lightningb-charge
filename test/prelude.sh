...
(command -v bitcoind && command -v brocoin-cli) > /dev/null || { echo >&2 "bitcoind and brocoin-cli must be in PATH. to install, see https://brocoin.org/en/full-node#linux-instructions"; exit 1; }
